...
const Notepreview = ({ setnotepreview, note, title, labels,fileName,fileurl }) => {
  ...
  const 
downloadURL="https://firebasestorage.googleapis.com/v0/b/notecloud-a73b3.appspot.com/o/uploads%2FFcs%20a2.pdf?alt=media&token=806b0710-d7ec-4d03-8024-8d535958586b"
...
<embed
      ...
      {fileName && (fileName.endsWith("png") || fileName.endsWith("jpg") || fileName.endsWith("jpeg") || fileName.endsWith("svg") || fileName.endsWith("gif")) ? (
        <>
        <h4 style={{marginBottom:"20px"}}>{fileName}</h4>
        
     <img style={{width:"calc(100% - 40px)",position:"absolute",left:"50%",transform:"translateX(-50%)"}} src={fileurl} alt="Uploaded" />
     </>
) : (
  ...
  fileName && (fileName.endsWith("pdf")) ? (
    ...
    <embed
          src={fileurl}
          style={{width:"calc(100% - 40px)",position:"absolute",left:"50%",transform:"translateX(-50%)",height:"600px"}}
          title="PDF Document"
        /> 
    </>
  ...
  download={fileName}
    target="_blank" 
    rel="noopener noreferrer" style={{textDecoration:"none",color:"lightblue"}}>Download {fileName}</a>
    </>
  ): (
    ""
  )
